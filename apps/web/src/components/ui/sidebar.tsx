...
/** CSS custom properties the sidebar sets on its own DOM nodes. */
...
function SidebarProvider({
  ...
  const toggleSidebar = React.useCallback(() => {
    return isMobile ? setOpenMobile((isOpen) => !isOpen) : setOpen(!open)
...
function SidebarMenuSkeleton({
  ...
  const skeletonStyle = React.useMemo<SidebarStyle>(() => {
    return { "--skeleton-width": `${Math.floor(Math.random() * 40) + 50}%` }
  }, [])
