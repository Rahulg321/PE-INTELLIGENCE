...
export function buildPreamble(ctx: CompanyContext): string {
    ...
    lines.push("2. Use researchCompany for external lookups only when you cannot answer from known data; each call costs budget, so spend it on the facts that matter for mandate fit.");
