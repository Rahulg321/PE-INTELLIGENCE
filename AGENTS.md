...
- Lint web: `cd apps/web && bun run lint`. Baseline is NOT clean — pre-existing errors in `src/components/ui/button.tsx`, `src/components/ui/alert.tsx`, `src/components/ui/sidebar.tsx` (`import/consistent-type-specifier-style`, plus `no-shadow` warnings in sidebar) and `src/features/auth/components/header-user.tsx` (`no-unnecessary-condition`); don't treat these as yours.
...
- **Schema is split per domain under `packages/db/src/schema/`** (repo convention): `auth.ts` (Better Auth tables `users`/`sessions`/`accounts`/`verifications` + `rate_limits`), `firms.ts` (onboarding: `firms`, `investment_mandates`, `mandate_sectors`, `mandate_criteria`), `posts.ts` (stale starter table), plus `relations.ts` (all `defineRelations`) and `index.ts` (re-exports). Add new entities to a new per-domain file in `src/schema/`.
...
- Onboarding: `apps/web/src/routes/onboarding.tsx` is a thin route (guarded by `beforeLoad`: unauthenticated → `/`, already-onboarded → `/dashboard`) rendering the 4-step wizard (firm basics → strategy → sectors → preferences) from `apps/web/src/features/onboarding/components/onboarding-wizard.tsx`. Submits via `saveOnboarding` (`features/onboarding/server/mutations/save-onboarding.ts`, a `createServerFn` that checks the session then delegates to `onboardingService.save` in `features/onboarding/server/onboarding-service.ts`, which persists firm + mandate + sectors + criteria in one `db.transaction`). `apps/web/src/routes/dashboard.tsx` (thin, renders `features/dashboard/components/dashboard-page.tsx`) shows the resulting mandate.
...
- **Current feature domains:** `features/auth/` (components `login-page.tsx`/`header-user.tsx`, session `client.ts`, server `get-session-user.ts` with `getSessionUser`/`requireUser`, `server/queries/get-session-status.ts`), `features/onboarding/` (components `onboarding-wizard.tsx`, `schemas.ts`, `constants.ts`, server `onboarding-service.ts`, `queries/get-onboarding-status.ts`, `mutations/save-onboarding.ts`), `features/dashboard/` (components `dashboard-page.tsx`, server `dashboard-service.ts`, `queries/get-dashboard-data.ts`), `features/mcp/` (MCP server, todos, handler).
- **Server fns live next to their feature as thin boundaries.** Each feature has `server/queries/*` and `server/mutations/*` (the `createServerFn`s — they own auth via `requireUser`/`getSessionUser` and validation) which delegate persistence to `server/*-service.ts`. The service imports `db` from `db`; client components must never import `db` or `#/lib/auth`. Shared server helper: `getSessionUser`/`requireUser` in `features/auth/server/get-session-user.ts`.
...
- Server fns are thin: `requireUser()`/`getSessionUser()` (from `features/auth/server/get-session-user.ts`) + zod validator + delegate to the service.
